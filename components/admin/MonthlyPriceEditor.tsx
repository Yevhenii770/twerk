...
export default function MonthlyPriceEditor({ classType, price }: { classType: string; price: number }) {
  ...
  const save = () => {
    ...
    startTransition(async () => { await updateMonthlyPrice(classType, parsed); router.refresh(); setEditing(false) })
  }

  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 18px', border: '1px solid #E0E0E0', background: '#fff' }}>
      ...
      {editing ? (
        <>
          ...
          <button onClick={() => { setValue(String(price)); setEditing(false) }} style={miniBtn('#888')}>Cancel</button>
        </>
      ) : (
