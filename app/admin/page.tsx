    ...
    <div className="light-page" style={{ minHeight: "100vh", background: "#fff", color: "#111", fontFamily: "inherit" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", borderBottom: "2px solid #111" }}>
        {[
          ...
          <div key={s.label} style={{ padding: "24px 32px", borderRight: "2px solid #111", background: s.bg }}>
            <p style={{ fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: s.text, marginBottom: 6, opacity: 0.7 }}>{s.label}</p>
            <p style={{ fontSize: 40, fontWeight: 800, color: s.text, margin: 0, lineHeight: 1 }}>{s.value}</p>
          </div>
        ))}
      </div>

      {/* Filter tabs */}
      <div style={{ display: "flex", borderBottom: "2px solid #111", background: "#F5F5F5" }}>
        {tabs.map(t => (
          <Link
            key={t.key}
      ...
      {/* Table */}
      {filtered.length === 0 ? (
        <div style={{ padding: "48px 32px", color: "#666", fontSize: 14 }}>No bookings yet.</div>
      ) : (
                ...
                <tr key={b.id} style={{ borderBottom: "1px solid #DDD", background: i % 2 === 0 ? "#fff" : "#FAFAFA" }}>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {b.status === "pending" && (
                        <StatusButton id={b.id} status="confirmed" label="✓ Confirm" bg="#1565C0" />
                      )}
                      {b.status === "confirmed" && (
                        <StatusButton id={b.id} status="paid" label="✓ Paid" bg="#2E7D32" />
                      )}
                      {b.status !== "cancelled" && (
                        <StatusButton id={b.id} status="cancelled" label="✕ Cancel" bg="#555" />
                      )}
