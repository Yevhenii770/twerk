...
// TEMPORARY: while the Square checkout is being tested/fixed, /book is showing the old
// phone-confirm form again (createBooking below) instead of BookingFlow. The Square-based
// flow (checkout.ts, BookingFlow.tsx) is untouched and ready to switch back — see the
// square-payments-checkpoint branch for the state right before this revert.
...
export async function createBooking(_: unknown, formData: FormData) {
  ...
  const { name, phone, classType, bookingType, date } = parsed.data;
  ...
  try {
    ...
    await sendTelegramNotification({ name, phone, classType, bookingType, date, price });
